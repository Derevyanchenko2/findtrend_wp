		...
		<?php if (!empty($main_tabs)): ?>
			<div class="main-tabs">
				...
				<?php foreach ($main_tabs as $index => $tab): ?>
					<div class="main-tab 
						<?php echo esc_attr($tab_classes[$index] ?? ''); ?> 
						<?php echo esc_attr($tab['main_tab_icon']['alt']); ?> 
						<?php echo esc_attr($main_tab_class); ?>"> 

						<div class="main-tab__logo">
							<img src="<?php echo esc_url($tab['main_tab_icon']['url']); ?>" 
								alt="<?php echo esc_attr($tab['main_tab_icon']['alt'] ?? 'icon'); ?>">
						</div>

						<span class="main-tab__title">
							<?php echo esc_html($tab['main_tab_text']); ?>
						</span>

						<div class="close main-tab__close">
							<img src="<?php echo get_template_directory_uri(); ?>/assets/images/close-icon-shadow.svg" alt="close icon">
						</div>
					</div>
				<?php endforeach; ?>
			</div>
        ...
        <div class="tabs-wrapper">
            <div class="tabs-content">
                <h2 class="section-title tabs-content__title">
                    <?php echo $optimization_title; ?>
                </h2>
                <img class="tabs-arrow__icon tabs-arrow__icon--left" src="<?php echo get_template_directory_uri(); ?>/assets/images/arrow-left-icon.svg" alt="">
						...
						<div class="tabs-notification">
								<div class="tabs-notification__btn">
									<div class="tabs-notification__logo">
										<img src="<?php echo ($tab['optimization_tab_icon']['url']); ?>" alt="<?php echo esc_attr($tab['optimization_tab_icon']['alt']); ?>">
									</div>
                ...
                <img class="tabs-arrow__icon tabs-arrow__icon--right" src="<?php echo get_template_directory_uri(); ?>/assets/images/arrow-right-icon.svg" alt="">
                <p class="section-text tabs-text">
                    <?php echo $optimization_text; ?>
                </p>
            </div>
        </div>
    </div>
         ...
         <div class="logos-wrapper row">
            <?php if( ! empty($logos_repeater) ): ?>
                ...
                <div class="logo-company">
                    ...
                    <img src="<?php echo esc_url($logo['logo']['url']); ?>" alt="<?php echo esc_attr($alt); ?> ?>">
                </div>
                <?php endforeach; ?>
            <?php endif; ?>
        </div> 
...
<section class="social-tabs-section section">
    <div class="container">
        <h2 class="section-title tabs-title"><?php echo $tabs_title; ?></h2> <!-- Заголовок вкладок -->
        
        <div class="tabs">
            <?php if( ! empty($social_tabs) ): ?>
                <?php foreach( $social_tabs as $index => $tab ): ?>
                    <button class="tab <?php echo ($index === 0) ? 'active' : ''; ?>" data-tab="<?php echo $tab['data_id']; ?>">
                        <img src="<?php echo $tab['socials_icon']['url']; ?>" alt="<?php echo $tab['socials_icon']['alt']; ?>">
                    </button>
                <?php endforeach; ?>
            <?php endif; ?>
        </div>
        ...
        <div class="tab-contents">
            <?php if( ! empty($social_tabs) ): ?>
                <?php foreach( $social_tabs as $index => $tab ): ?>
                    <div class="tab-content <?php echo ($index === 0) ? 'active' : ''; ?>" id="<?php echo $tab['data_id']; ?>">
                        <?php 
                        // Получаем повторитель tab_content внутри каждого tabz
                        $tab_content = $tab['tab_content'];
                        
                        if( ! empty($tab_content) ):
                            foreach( $tab_content as $content_image ): ?>
                                <div class="tweet-image">
                                    <img src="<?php echo $content_image['tab_content_image']['url']; ?>" alt="<?php echo $content_image['tab_content_image']['alt']; ?>">
                                </div>
                            <?php endforeach;
                        endif;
                        ?>
		...
		?>
			<div class="pricing-wrapper row">
				<?php while ($packages->have_posts()) : $packages->the_post(); 
					...
					$highlight_class = ($counter == 2) ? ' highlighted' : '';
					$highlight_btn_class = ($counter == 2) ? ' highlighted-btn' : 'pricing-btn--green';
				?>
					<div class="pricing-column">
						<div class="pricing-item<?php echo $highlight_class; ?>">
							...
							<p class="price" data-monthly="<?php echo esc_attr($price_monthly); ?>" data-yearly="<?php echo esc_attr($price_yearly); ?>"><?php echo esc_html($price_monthly); ?> <span>/Month</span></p>
							...
							<a href="<?php echo $tariff_btn_url['url']; ?>" class="btn pricing-btn <?php echo $highlight_btn_class; ?>"><?php echo ($tariff_btn_text); ?></a>
						</div>
					</div>
				<?php endwhile; ?>
			</div>
	...
	<div class="container">
		<div class="join-heading">
			<div class="join-heading__title">
				<h2 class="section-title--large"><?php echo $join_title; ?></h2>
